...
- **Messaging:** Single message form with media upload/URL support, automated logging, and job-dispatch to the Node gateway.
- **Broadcasts:** Upload CSV/XLSX recipient lists (up to 500 numbers per batch), tune the delay, monitor progress, and review recent attempts.
- **Auto replies:** Define per-device, keyword-based rules with exact/contains matching and template/text responses.
- **Logs & notifications:** Filterable message ledger plus a notification dropdown that captures system/device/message/broadcast events.
- **Webhooks & API:** Incoming messages and device events persist to `message_logs`, `panel_notifications`, and trigger auto replies as needed, ready for a Baileys worker to consume.
